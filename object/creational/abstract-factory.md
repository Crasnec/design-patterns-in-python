...
개발자가 `WarriorEquipmentFactory` 구현 시 실수로 `Wand`를 반환하도록 작성하더라도 인터페이스 규칙(`Weapon` 반환)은 만족하므로 정적 에러가 발생하지 않습니다. 즉, 추상 팩토리는 **타입 수준에서 조합을 완벽히 강제한다기보다는, 관련 객체 생성을 한 클래스로 묶어 실수를 줄이는 아키텍처 접근법**으로 이해하는 것이 정확합니다.
