...
즉, 기존 가변 객체 환경에서 "동일 객체 공유로 인한 사이드 이펙트 문제"는 불변 데이터 모델을 채택하는 순간 구조적으로 발생하지 않게 됩니다.
...
결론적으로 현대적인 관점에서 프로토타입 패턴의 본질을 재정의하자면, "이미 존재하는 값을 새로운 값 생성의 기준으로 삼아, 상태의 공유·복제·소유권·정체성을 안정적으로 모델링하는 설계 기법"으로 확장하여 다룰 수 있습니다.