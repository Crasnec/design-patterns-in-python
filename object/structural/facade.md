...
별도의 Wrapper 클래스 없이도 타입의 가시성 제한을 통해 Facade 경계를 구축할 수 있습니다.
...
클라이언트는 필요하지 않은 불필요한 내부 접근 권한을 애초에 부여받지 못하게 됩니다.
...
모듈 경계를 활용하면 서브시스템의 세부 구현을 완전히 감출 수 있습니다.
...
```python
def start(facade: GameFacade[Created], config: ConfigPath) -> GameFacade[Running]: ...
def stop(facade: GameFacade[Running]) -> GameFacade[Stopped]: ...

```

올바르지 않은 상태에서의 메서드 호출을 컴파일 시점에 완벽히 차단합니다.
...
고전적 관점의 파사드 패턴은 복잡한 서브시스템 앞에 단순하고 목적 중심적인 통합 인터페이스를 배치하여, 클라이언트가 내부 세부 구조를 알지 못해도 주요 기능을 사용할 수 있게 돕는 구조 패턴입니다.

현대적인 타입 시스템과 함수형 패러다임의 관점에서 파사드의 본질을 재해석한다면 다음과 같이 정리할 수 있습니다.

> **"복잡한 시스템이 제공하는 넓은 기능, 타입, 오류, 효과의 표면에서, 특정 클라이언트가 실제로 필요로 하는 최소한의 고수준 의미만을 추출하여 명시적 경계로 제공하는 기법"**